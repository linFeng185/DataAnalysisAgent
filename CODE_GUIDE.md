...
│   │   └── providers/        数据源提供者
│   ├── connectors/       ⑤ 数据库连接器 — 6 种方言（ClickHouse HTTP / Oracle service_name）
...
│   ├── db/               ⑫ 状态库基础设施 — 版本化 PostgreSQL 迁移执行器
...
| 文件 | 职责 |
|------|------|
| `routes.py` | 查询、历史、Schema、数据源、Skills、知识库、health 等管理端点 |
...
| `workflow.py` | 16 节点 StateGraph + 意图/Schema/跨轮恢复/校验/执行条件路由 + Checkpointer |
...
`client.py`（全局工厂 + local/remote/none 任务路由）+ `adapters/`（DeepSeek/OpenAI/Anthropic 适配器）+ `prompts.py`。默认只有 `generate_sql` 可调用远程模型，轻量节点优先 `LOCAL_LLM_*`。

### ④ `src/datasource/` — 数据源管理

`registry.py`（全局 Provider、Oracle/ClickHouse 方言适配、`resolve_or_none` 不抛异常；ClickHouse 按 `extra_params.connect_timeout` 做 TCP 建连探针）+ `credential_manager.py`（Fernet + 环境变量凭证）+ `setup.py`（SQLite 演示库）。

### ⑤ `src/connectors/` — 数据库连接器

6 种方言：ClickHouse（clickhouse-connect HTTP）/ MySQL / PostgreSQL / SQLite / Oracle（service_name）/ MSSQL。
...
| `upload_manager.py` | 异步上传任务、范围/标签 metadata 和后台 VectorStore 写入 |
...
| `history_store.py` | PostgreSQL 查询历史 + 内存环形缓冲回退；final_result JSONB 持久化逐轮 SQL、数据、分析、图表和推理 |
...
- **安全阻断**：layer3 使用 sqlglot AST 只读白名单，DDL/DML 在进入数据库前被拦截
