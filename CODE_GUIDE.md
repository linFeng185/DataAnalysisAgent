...
```
├── src/
│   ├── api/              ① Web 接口层 — 领域路由包/认证/管理端点 + SSE 流式
...
6 种方言通过 `connectors/registry.py` 自注册：ClickHouse / MySQL / PostgreSQL / SQLite / Oracle / MSSQL。超时、EXPLAIN、探针和 Engine 参数由各 Connector 封装。
