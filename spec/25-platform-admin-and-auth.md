...
- 登录继续使用 HttpOnly Cookie；生产 Cookie 仅通过 HTTPS 发送。
- 登录先执行客户端地址与规范化用户名组合的滑动窗口限流，再校验数据库账号锁定状态。
...
- `registration_enabled` 默认关闭。关闭时 `POST /auth/register` 返回 403，`GET /auth/me`
  返回该开关供前端决定是否展示注册入口。
- 公开注册只在默认租户创建 `analyst`，不能创建租户或管理员。租户和租户管理员由平台后台
  原子创建。
...
| GET | `/api/v1/admin/users` | 按租户、角色和状态列出用户 |
| POST | `/api/v1/admin/users` | 创建租户用户，禁止创建第二个 `super_admin` |
| PATCH | `/api/v1/admin/users/{user_id}` | 修改角色或启停账号 |
| POST | `/api/v1/admin/users/{user_id}/reset-password` | 重置密码并清除锁定 |
...
- 登录页根据 `registration_enabled` 显示或隐藏注册表单。
