...
修复当前项目审计发现的安全、正确性、API 生命周期和可运维性缺陷。所有环境强制登录，
`MULTI_TENANT` 仅控制租户隔离；生产环境必须在启动阶段拒绝不完整的认证、凭证和数据库安全配置。

## 兼容边界

| 环境 | 认证行为 | 说明 |
|------|----------|------|
| `dev` / `test` 且 `MULTI_TENANT=false` | JWT 必须有效 | 使用默认租户的单体部署 |
| 任意环境 `MULTI_TENANT=true` | JWT 必须有效 | 所有租户资源按用户和租户隔离 |
| `prod` | JWT、`JWT_SECRET`、凭证密钥、状态数据库配置必须存在 | 配置缺失时启动失败 |
...
1. 数据源凭证采用 `v2:<base64-salt>:<fernet-token>` 格式。每次加密生成独立 16 字节随机
   salt，salt 与密文共同保存；解密继续兼容历史固定 salt 的 Fernet token 和旧明文配置。
   生产环境禁止缺省主密钥，避免随机 salt 掩盖弱主密钥问题。
2. SQL AST 安全层除 DDL/DML 外，继续阻断 PostgreSQL/MSSQL `SELECT INTO` 和
   `setval`/`nextval` 等已知状态变更函数。表名幻觉校验、执行前列引用校验或方言解析异常时
   一律失败关闭；MSSQL 使用 sqlglot `tsql` 方言。
3. 自动建库和枚举采样无法参数化的数据库、表、列标识符必须按目标数据库规则引用；采样
   数量继续使用绑定参数。Connector URL 使用 SQLAlchemy `URL.create()`，日志和异常默认
   隐藏密码，同时正确处理密码中的 `@`、`/`、`:` 等字符。
4. Milvus 的 JSON 字符串 LIKE 只作为候选过滤，用户值通过 JSON 字面量转义；搜索、读取、
   计数和删除必须解析 metadata 后做 Python 精确匹配，避免租户或数据源前缀误命中。
5. 文档磁盘回退以 `realpath + commonpath` 验证系统知识根目录。上传任务只保留有界数量，
   完成/失败任务按 TTL 清除；知识检索、MCP 列表和会话恢复故障向客户端返回稳定状态，完整
   异常只进入日志，禁止把故障伪装成空结果。
6. 查询历史 PostgreSQL 写入由工作流显式 `await`，失败进入调用链日志；外部动作审计使用
   有界缓冲，幂等键容量耗尽时拒绝新执行而不淘汰旧键，避免旧动作被重复执行。
7. 生产环境缺少 JWT 密钥时签发直接失败；Swagger、Redoc 和 `/openapi.json` 同时关闭。
   模型连通性端点使用有长度和字符集限制的 Pydantic 请求，测试请求固定为单 token。
