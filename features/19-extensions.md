...
## 19. 扩展能力 (Phase 4) `[P2:9]` `[P3:12]`

| # | 功能 | 描述 | 状态 |
|---|------|------|------|
| 19.1 | 自动 Insight 发现 | 定期 (每小时/每天) 扫描数据，主动推送异常和趋势变化 | 集成测试完成 |
| 19.2 | 定时报告 | 按日/周/月自动生成分析报告并推送 (邮件/飞书/Slack) | 集成测试完成 |
| 19.3 | 知识库自进化 | 高质量 SQL 自动积累，从 AUTO_INTROSPECT 升级到 LEARNED_PATTERN | 待开发 |
| 19.4 | 多模态输入 | 上传 Excel/CSV/图片 → 自动建临时表/视觉分析 (见 §15.2) | 待开发 |
| 19.5 | 权限体系 | 多用户注册/登录，RBAC + 行级/列级权限 + RLS + 审计日志 (见 §15.8) | 单测完成 |
...
| 19.11 | 多模型适配 | Provider 注册表 + 运行时切换 + 4 模型 | 开发完成 | P2 |
| 19.12 | 多数据源交叉分析 | multi_source 并行 + merge_results LLM 合并 | 开发完成 | P2 |
| 19.13 | 非查询式回答 | metadata/chat 意图不含 SQL 流水线，知识库/对话直接回答 | 开发完成 | P2 |
| 19.14 | 联网搜索分析 | MCP 封装搜索 API → 结构化 → 分析，不依赖预配数据源 (见 §15.5) | 待开发 | P2 |
| 19.15 | SQL 优化 Skills | 索引感知 + 重写规则 + 性能模板 (见 §15.7) | 待开发 | P3 |
| 19.16 | 知识库增强 | 三范围知识、数据源公共文档、个人补充、标签治理与系统目录扫描（主实现见 6.9） | 单测完成 | P2 |
| 19.17 | VectorStore 抽象层 | ChromaDB/Milvus/pgvector 统一接口；Milvus 表达式转义后执行 metadata 精确过滤 | 单测完成 | P2 |
...
| 19.21 | 受控外部动作接口 | `src/actions/` | 外部动作必须人工确认、幂等键、审计和默认禁用，不包含自动交易 | 单测完成 | P2 |
| 19.22 | 统一扩展能力子图 | `src/graph/subgraphs/` | 文件、市场研究、预测、报告、外部动作独立执行并统一输出 AnalysisArtifact | 单测完成 | P2 |
...
本模块共 22 项，已完成 12 项，待开发 10 项。19.1/19.2 已完成 PostgreSQL 调度、运行时权限复核、站内/邮件/飞书/Slack 出站分发、API 和前端工作台；19.7 的 Bot 入站查询仍是独立待开发能力。

| 待开发项 | 不开发原因 | 可开发条件 | 预计开发时机 |
|---|---|---|---|
| 19.3-19.4、19.6、19.8-19.10、19.14-19.15、19.20 | 本轮未扩展自进化、多模态、联网研究和运维集成 | 明确业务触发、网络授权、隔离 worker 和验收集 | Phase 3/4，按对应能力评审后 |
| 19.7 | 当前仅实现服务端飞书/Slack 出站通知，未实现 Bot 入站查询 | 明确平台应用凭证、事件回调验签、会话身份映射和限流策略 | Phase 4，消息平台接入评审后 |
