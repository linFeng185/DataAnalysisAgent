...
class RequestIdentity:
    ...
    def system(cls, role: str = "super_admin") -> "RequestIdentity":
        ...
        logger.debug("构造系统身份完成", role=result.role)
        return result
...
def normalize_role(role: str | None) -> str:
    ...
    logger.debug("租户策略角色规范化完成", role=result)
    return result
...
class TenantPolicy:
    ...
    def datasource_isolation_enabled(self) -> bool:
        ...
        logger.debug("读取数据源隔离策略完成", enabled=result)
        return result
    ...
    def knowledge_isolation_enabled(self) -> bool:
        ...
        result = self.multi_tenant
        logger.debug("读取知识隔离策略完成", enabled=result)
        return result
    ...
    def requires_authentication(self, *, is_probe: bool = False) -> bool:
        ...
        result = not is_probe
        logger.debug("判断租户认证门禁完成", required=result)
        return result
    ...
    def tenant_filter(self, tenant_id: int, *, explicit: bool = False) -> dict[str, str]:
        ...
        logger.debug("生成租户过滤完成", applied=bool(result), tenant_id=tenant_id)
        return result
    ...
    def validate_identity(self, identity: RequestIdentity) -> RequestIdentity:
        ...
        invalid = (
            ...
            or (role == "super_admin" and identity.user_id != SUPER_ADMIN_USER_ID)
        )
        if invalid:
            logger.warning(
                "租户身份校验拒绝",
        ...
        logger.debug("校验租户身份完成", tenant_id=identity.tenant_id, user_id=identity.user_id)
        return identity
    ...
    def can_write_scope(self, scope: str, identity: RequestIdentity) -> bool:
        ...
        logger.debug("判断租户作用域写权限完成", scope=normalized_scope, allowed=result)
        return result
